...
if __name__ == "__main__":
    ...
    if config.n_folds >= 2 and gkf is not None and groups is not None:
        print("\n" + "=" * 60)
        print("CROSS-VALIDATION ON TRAINING DATA (SAE)")
        print("=" * 60)
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(
            ...
            # Train SAE model on clean training fold
    ...
    model_path = Path.cwd() / "resources" / "models" / "sae_model.pkl"
    clf.save(model_path)
    print(f"SAE Model saved to: {model_path}")

    # Save the Artefact Removal Object
    ar_path = Path.cwd() / "resources" / "models" / "sae_artefact_removal.pkl"
