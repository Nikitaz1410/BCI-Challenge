...
class mlm_mask(nn.Module):  
    def __init__(self, emb_size=256, depth=6, n_classes=2,mask_ratio=0.5, pretrain=None,pretrainmode=False):
        super().__init__()
    ...
    def init_from_pretrained(self, pretrained_path, freeze_encoder=False, strict=True):
        # Load weights on CPU to avoid CUDA requirement when no GPU is available
