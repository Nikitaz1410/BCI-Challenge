"""
Online BCI Pipeline for Advanced Deep Learning Models.

This script runs real-time EEG classification using a pre-trained Advanced
(DeepBCIModel) model. No online adaptation is performed—the model uses the
configuration trained offline.

Pipeline:
1. Load configuration and pre-trained model (advanced_best_model_*.pkl)
2. Connect to LSL streams (EEG + optional Markers)
3. Process incoming chunks: filter → artifact rejection → classify
4. Send predictions to BCIController (robust buffer + UDP to game)

Requires:
- Pre-trained model from main_offline_Advanced.py (advanced_best_model_*.pkl)
- ArtefactRemoval from same run (advanced_artefact_removal.pkl)
- LSL streams: EEG data, optional Markers for evaluation

Usage:
    python main_online_Advanced.py
"""
...
# Add src directory to Python path to allow imports when run directly
...
from bci.models.Advanced_Baselines import DeepBCIModel
...
class BCIEngine:
    ...
    def _init_models(self):
        """Load Advanced (DeepBCIModel), signal filter, and artifact rejection."""
        base_path = Path.cwd() / "resources" / "models"
        artifact_path = base_path / "advanced_artefact_removal.pkl"

        # Find Advanced model (advanced_best_model_*.pkl)
        model_files = sorted(base_path.glob("advanced_best_model_*.pkl"))
        if not model_files:
            raise FileNotFoundError(
                f"No Advanced model found in {base_path}. "
                "Run main_offline_Advanced.py first to train and save the model."
            )
        model_path = model_files[0]
        logger.info(f"Using Advanced model: {model_path.name}")

        if not artifact_path.exists():
            raise FileNotFoundError(
                f"ArtefactRemoval not found at {artifact_path}. "
                "Run main_offline_Advanced.py first."
            )

        # Signal Filter
        self.signal_filter = Filter(self.config, online=True)

        # Advanced Classifier (no online adaptation)
        self.clf = DeepBCIModel.load(str(model_path))
        logger.info(f"Loaded Advanced model from {model_path}")
    ...
    def run(self):
        ...
        logger.info("Starting BCI Loop (Advanced, no online adaptation)...")
        ...
        try:
            while self.running:
                ...
                # --- 4. Classification (no online adaptation) ---
                # DeepBCIModel expects (N, C, T); add batch dimension
                window_3d = self.buffer[np.newaxis, ...]
                probs = self.clf.predict_proba(window_3d)
