...
# Local modules
# =============================================================================
from src.dinoRun.helper.dinosaur import (
    Dinosaur,
    Task,
    DecayingBar,
    SmallCactus,
...
from src.dinoRun.helper.lsl_markerstream import LSLMarkerStream
from helpers import (
    config,
    get_current_marker,
...
SCREEN_WIDTH = 1200
SCREEN_HEIGHT = 800
GAME_FOLDER = join(os.getcwd(), "src", "dinoRun")
ASSETS = join(GAME_FOLDER, "assets")
...
class _Evidence:
    ...
    def add(self, t, label):
        self.t.append(t); self.y.append(int(label))
...
def new_session_simulation(success_rate: float = 0.6, num_trials: int = 10):
    ...
    print(f"Random number generated: {random_num:.2f} (success rate threshold: {min_success:.2f}, success rate: {success_rate:.2f})")
    random_success = random_num < success_rate
    print(f"Simulating session with success rate {success_rate:.2f}, min successes {min_success}, total trials {num_trials}")
    ...
    if random_success:
        ...
        trials = [-2]*num_success + [-1]*(num_trials - num_success)  # Simulate successes and failures
    else:
        num_success = np.random.randint(1, min_success)
        trials = [-2]*num_success + [-1]*(num_trials - num_success)
    print(f"Simulated {num_success} successes out of {num_trials} trials with success rate {success_rate:.2f}")
...
# =============================================================================
def log_trial_metrics(user_id: str, mode: str, cue: str | None, marker: str | None,
                      success: int, t_success: float | None,
                      total_preds: int, target_votes: int, frac_target: float,
                      consensus: float, max_time: float) -> None:
    out_dir = os.path.join(os.getcwd(), "data", f"sub-{user_id}")
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, "dino_trial_metrics.csv")
    header = ["timestamp","mode","cue","marker","success","time_to_success_s",
              "total_votes","target_votes","frac_target","consensus","max_time_s"]
    row = [datetime.datetime.now().isoformat(timespec="milliseconds"), mode, cue, marker,
           success, (round(t_success,3) if t_success is not None else ""),
           total_preds, target_votes, round(frac_target,3), round(consensus,3), max_time]
...
def success_rate_last_session(user_id):
    file_path = os.path.join(os.getcwd(), "data", f"sub-{user_id}", "dino_bar_values_and_jumps.csv")
    rows = []
    ...
    print(f"Session {last_date}: {rate:.1%} success over {len(session_successes)} jumps")
    
# =============================================================================
# Main loop
# =============================================================================
def main() -> None:
    global game_speed, x_pos_bg, y_pos_bg, points, obstacles, death_count, marker_names, SCREEN_WIDTH, SCREEN_HEIGHT
    ...
    def quicktime() -> None:
        ...
        def handle_decaying_bar(decay_bar: DecayingBar, cue_idx: int) -> tuple[int, int, str | None]:
            nonlocal in_quicktime, player, lost, font, sim_predicted, cue, time_now, sim_pred, session_use_udp
            ...
            while in_quicktime:
                ...
                if not use_live_udp:
                    ...
                    if new_command:
                        ...
                        if cmd_idx is not None and marker_names[cmd_idx] == cue:
                            decay_bar.bump()
                            print(f"UDP bump at {now:.2f} for cue {cue} (index {cmd_idx})")
        ...
        decay_bar = DecayingBar(x=SCREEN_WIDTH - 175, y=SCREEN_HEIGHT - 300, width=50, height=height, color=(240, 128, 128), max_value=max_value, decay_rate=0.4, bump_rate=18) # those are magic numbers, do not change, its based on how much needed to match bump and accuracy
        total_cmds, matching_cmds, last_cmd_label = handle_decaying_bar(decay_bar, marker_names.index(cue))
        ...
        print(f"Quicktime summary ({mode_text}) - accuracy {accuracy_text}, last UDP command: {last_label_text}")
    ...
    while run:
        ...
        if len(obstacles) == 0:
            ...
            obstacles.append(
                SmallCactus(SCREEN_WIDTH, pygame) if r == 0 else
                LargeCactus(SCREEN_WIDTH, pygame) if r == 1 else
                Bird(SCREEN_WIDTH, pygame)
            )
        ...
        if GAME_MODE == "health":
            SCREEN.blit(font.render(f"Health: {player.health}", True, (240, 128, 128)), (10, 40))
        else:
            time_remaining = max(0, TIME_LIMIT - (time.time() - game_start_time))
            SCREEN.blit(font.render(f"Time: {int(time_remaining)}s", True, (240, 128, 128)), (10, 40))
            SCREEN.blit(font.render(f"Health: {player.health}", True, (240, 128, 128)), (10, 70))
        ...
        for obstacle in obstacles[:]:
            ...
            if gametype == "quicktime" and not lost and not player.invulnerable and not cues_empty:
                if (obstacle.ideal_jump - player.dino_rect.x < 150) and not player.dino_jump:
                    run = False
                    #paused()
                    ...
                    _, last_payload_ts, last_payload_valid, _ = get_udp_payload_snapshot()
                    has_recent_valid = (
                        last_payload_valid
                    ...
                    if session_use_udp is None:
                        ...
                        mode_text = "live UDP commands" if session_use_udp else "simulated quicktime predictions"
                        print(f"Session input mode locked to {mode_text}.")
                        if listener_bound and not has_data and has_recent_valid:
                            print("Recent UDP packet detected but marked invalid; gameplay will stay in simulation until a recognized command arrives.")
                    elif session_use_udp is False and listener_bound and has_data:
                        print("Live UDP commands detected but session is locked to simulation; restart the game to switch modes.")
                    if session_use_udp:
                        sim_pred = []
                    else:
...
def menu(initial_death_count: int) -> None:
    ...
    def get_user_id() -> str:
        ...
        def attempt_start_game() -> None:
            nonlocal input_active, port_input, port_feedback, port_feedback_color, port_feedback_time
        ...
        def udp_config_screen() -> None:
            ...
            while config_active:
                ...
                pygame.draw.rect(SCREEN, (60, 90, 150), back_button_rect, border_radius=12)
                SCREEN.blit(small_font.render("Back", True, (255, 255, 255)), (back_button_rect.x + 54, back_button_rect.y + 14))

                panel_width = min(680, SCREEN_WIDTH - 80)
                panel_height = min(520, SCREEN_HEIGHT - 80)
                panel_x = (SCREEN_WIDTH - panel_width) // 2
                panel_y = (SCREEN_HEIGHT - panel_height) // 2
                udp_panel_rect = pygame.Rect(panel_x, panel_y, panel_width, panel_height)

                content_x = udp_panel_rect.x + 24
                content_width = udp_panel_rect.width - 48
                header_height = 68

                status_rect = pygame.Rect(content_x, udp_panel_rect.y + header_height + 16, content_width, 76)
                command_rect = pygame.Rect(content_x, status_rect.bottom + 14, content_width, 70)
                port_rect = pygame.Rect(content_x, command_rect.bottom + 16, content_width, 150)
                marker_rect_height = 86
                marker_rect = pygame.Rect(content_x, udp_panel_rect.bottom - 24 - marker_rect_height, content_width, marker_rect_height)
                log_rect = pygame.Rect(content_x, port_rect.bottom + 16, content_width, marker_rect.y - 24 - (port_rect.bottom + 16))
                if log_rect.height < 48:
                    log_rect.height = 0

                port_input_rect = pygame.Rect(port_rect.x + 12, port_rect.y + 46, 150, 48)
                apply_port_rect = pygame.Rect(port_input_rect.right + 12, port_input_rect.y, 170, 48)
                listener_button_rect = pygame.Rect(port_rect.x + 12, port_input_rect.bottom + 12, content_width - 24, 48)

                listener_bound, udp_has_data, listener_port, packet_log = get_udp_listener_status()
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_ESCAPE:
                            ...
                            elif len(event.unicode) == 1 and event.unicode.isdigit() and len(port_input) < 5:
                ...
                listener_bound, udp_has_data, listener_port, packet_log = get_udp_listener_status()
                listener_running = is_udp_thread_running()
                payload_text, payload_ts, payload_valid, payload_label = get_udp_payload_snapshot()

                pygame.draw.rect(SCREEN, (33, 35, 46), udp_panel_rect, border_radius=20)
                pygame.draw.rect(SCREEN, (80, 100, 140), udp_panel_rect, 2, border_radius=20)
                header_rect = pygame.Rect(udp_panel_rect.x, udp_panel_rect.y, udp_panel_rect.width, header_height)
                pygame.draw.rect(SCREEN, (60, 90, 150), header_rect, border_top_left_radius=20, border_top_right_radius=20)
                SCREEN.blit(small_font.render("UDP Configuration", True, (255, 255, 255)), (header_rect.x + 22, header_rect.y + 18))

                blink_on = (pygame.time.get_ticks() // 400) % 2 == 0
                pygame.draw.rect(SCREEN, (46, 52, 66), status_rect, border_radius=16)
                indicator_center = (status_rect.x + 24, status_rect.y + status_rect.height // 2)
                ...
                SCREEN.blit(tiny_font.render("Status", True, (220, 220, 220)), (indicator_center[0] + 20, status_rect.y + 12))
                SCREEN.blit(tiny_font.render(status_line, True, (200, 200, 200)), (indicator_center[0] + 20, status_rect.y + 38))
                if listener_bound:
                    port_status_line = f"Bound to 127.0.0.1:{listener_port}"
                else:
                    state = "running" if listener_running else "stopped"
                    port_status_line = f"Configured port {port_input or listener_port} ({state})"
                SCREEN.blit(tiny_font.render(port_status_line, True, (180, 200, 220)), (indicator_center[0] + 20, status_rect.y + 56))

                if payload_ts > 0:
                    age = time.time() - payload_ts
                    if age <= 10:
                        pygame.draw.rect(SCREEN, (45, 49, 63), command_rect, border_radius=16)
                        SCREEN.blit(small_font.render("Last command", True, (220, 220, 220)), (command_rect.x + 12, command_rect.y + 10))
                        ...
                        SCREEN.blit(tiny_font.render(label_text, True, (200, 200, 200)), (command_rect.x + 12, command_rect.y + 30))
                        SCREEN.blit(tiny_font.render(f"{validity} command - {age:.1f}s", True, val_color), (command_rect.x + 12, command_rect.y + 46))
                    else:
                        pygame.draw.rect(SCREEN, (55, 30, 30), command_rect, border_radius=16)
                        SCREEN.blit(small_font.render("Last command", True, (255, 200, 200)), (command_rect.x + 12, command_rect.y + 10))
                        SCREEN.blit(tiny_font.render("No recent commands (>10s)", True, (255, 180, 180)), (command_rect.x + 12, command_rect.y + 34))
                        SCREEN.blit(tiny_font.render("Check your sender.", True, (220, 160, 160)), (command_rect.x + 12, command_rect.y + 50))
                else:
                    pygame.draw.rect(SCREEN, (45, 49, 63), command_rect, border_radius=16)
                    SCREEN.blit(small_font.render("Last command", True, (220, 220, 220)), (command_rect.x + 12, command_rect.y + 10))
                    SCREEN.blit(tiny_font.render("No packets yet.", True, (130, 130, 130)), (command_rect.x + 12, command_rect.y + 32))

                pygame.draw.rect(SCREEN, (45, 49, 63), port_rect, border_radius=16)
                SCREEN.blit(small_font.render("UDP Port", True, (220, 220, 220)), (port_rect.x + 12, port_rect.y + 12))
                hint_text = "Stop listener to edit." if listener_running else "Pick a port, then start."
                SCREEN.blit(tiny_font.render(hint_text, True, (170, 190, 210)), (port_rect.x + 14, port_rect.y + 36))

                border_color = (90, 180, 140) if udp_active_input == "port" and not listener_running else (90, 90, 90)
                pygame.draw.rect(SCREEN, border_color, port_input_rect, border_radius=12)
                pygame.draw.rect(SCREEN, (15, 15, 15), port_input_rect.inflate(-6, -6), border_radius=10)
                SCREEN.blit(small_font.render(port_input or "5005", True, (255, 255, 255)), (port_input_rect.x + 12, port_input_rect.y + 12))
                ...
                SCREEN.blit(tiny_font.render("Apply", True, (255, 255, 255)), (apply_port_rect.x + 64, apply_port_rect.y + 14))

                listener_btn_color = (80, 200, 140) if not listener_running else (200, 120, 80)
                pygame.draw.rect(SCREEN, listener_btn_color, listener_button_rect, border_radius=14)
                listen_label = "Start Listener" if not listener_running else "Stop Listener"
                SCREEN.blit(small_font.render(listen_label, True, (255, 255, 255)), (listener_button_rect.x + 18, listener_button_rect.y + 14))

                if port_feedback and (time.time() - port_feedback_time) < 4.0:
                    SCREEN.blit(tiny_font.render(port_feedback, True, port_feedback_color), (port_rect.x + 14, listener_button_rect.bottom + 6))

                if log_rect.height >= 60:
                    pygame.draw.rect(SCREEN, (42, 45, 58), log_rect, border_radius=14)
                    SCREEN.blit(small_font.render("Recent packets", True, (220, 220, 220)), (log_rect.x + 12, log_rect.y + 10))
                    now_ts = time.time()
                    ...
                    if entries:
                        for payload_text_i, ts_i, recognized_i, label_i in entries:
                            ...
                            label_display = label_i or truncate_payload(payload_text_i, 44)
                            line = f"[{flag}] {age_i:.1f}s {label_display}"
                            SCREEN.blit(tiny_font.render(line, True, (200, 200, 200)), (log_rect.x + 12, line_y))
                            line_y += 18
                    else:
                        SCREEN.blit(tiny_font.render("No packets recorded.", True, (130, 130, 130)), (log_rect.x + 12, line_y))

                pygame.draw.rect(SCREEN, (42, 45, 58), marker_rect, border_radius=14)
                SCREEN.blit(small_font.render("Marker stream", True, (220, 220, 220)), (marker_rect.x + 12, marker_rect.y + 10))
                marker_info = ""
                if "lsl_markerstream" in globals():
                    marker_info = f"LSL: {lsl_markerstream.name} (UID {lsl_markerstream.marker_id})"
                SCREEN.blit(tiny_font.render(marker_info or "LSL stream initialising...", True, (200, 200, 200)), (marker_rect.x + 14, marker_rect.y + 34))
                SCREEN.blit(tiny_font.render("Markers: " + " | ".join(marker_names), True, (200, 200, 200)), (marker_rect.x + 14, marker_rect.y + 52))
        ...
        while input_active:
            ...
            udp_summary_rect = pygame.Rect(summary_x, summary_y, summary_width, summary_height)
            udp_open_rect = pygame.Rect(summary_x, udp_summary_rect.bottom + 16, summary_width, 56)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_TAB:
                        ...
                        attempt_start_game()
                    elif event.key == pygame.K_BACKSPACE and active_input == "user" and user_id:
                        user_id = user_id[:-1]
                    elif active_input == "user" and len(event.unicode) == 1 and event.unicode.isprintable():
            ...
            SCREEN.fill((0, 0, 0))
            label = "Enter your user ID:" if language == "eng" else "Geben Sie Ihre Benutzer-ID ein:"
            SCREEN.blit(font.render(label, True, (255, 255, 255)), (13, SCREEN_HEIGHT // 2 - 110))

            input_border_color = (90, 180, 140) if active_input == "user" else (60, 60, 60)
            pygame.draw.rect(SCREEN, input_border_color, input_rect, border_radius=12)
            pygame.draw.rect(SCREEN, (25, 25, 25), input_rect.inflate(-6, -6), border_radius=10)
            ...
            SCREEN.blit(font.render(shown, True, color), (input_rect.x + 10, input_rect.y + 28))

            lang_label = "English" if language == "eng" else "Deutsch"
            lang_text = "Selected Language: " if language == "eng" else "Ausgewählte Sprache: "
            SCREEN.blit(font.render(f"{lang_text}{lang_label}", True, (255, 255, 255)), (10, SCREEN_HEIGHT // 2 + 90))
            ...
            SCREEN.blit(font.render(lang_btn_text, True, (255, 255, 255)), (lang_button_rect.x + 16, lang_button_rect.y + 6))

            SCREEN.blit(font.render(f"Session: {session}", True, (255, 255, 255)), (10, SCREEN_HEIGHT // 2 + 180))
            ...
            SCREEN.blit(font.render("-", True, (255, 255, 255)), (sess_minus_rect.x + 18, sess_minus_rect.y + 6))
            SCREEN.blit(font.render("+", True, (255, 255, 255)), (sess_plus_rect.x + 16, sess_plus_rect.y + 6))

            start_btn_color = (60, 200, 160) if user_id else (80, 80, 80)
            pygame.draw.rect(SCREEN, start_btn_color, start_game_button, border_radius=16)
            start_text = "Start Dino Game" if language == "eng" else "Starte Dino Spiel"
            SCREEN.blit(font.render(start_text, True, (255, 255, 255)), (start_game_button.x + 28, start_game_button.y + 28))
            ...
            pygame.draw.rect(SCREEN, (70, 90, 140), udp_summary_rect, 2, border_radius=18)
            ...
            SCREEN.blit(small_font.render("UDP status", True, (220, 220, 220)), (status_indicator_center[0] + 18, udp_summary_rect.y + 14))
            SCREEN.blit(tiny_font.render(summary_status, True, (200, 200, 200)), (status_indicator_center[0] + 18, udp_summary_rect.y + 36))

            configured_port = port_input or str(listener_port)
            port_line = f"Port {listener_port} (running)" if listener_running else f"Port {configured_port}"
            SCREEN.blit(tiny_font.render(port_line, True, (190, 200, 220)), (udp_summary_rect.x + 18, udp_summary_rect.y + 68))
            ...
            if listener_bound and payload_ts > 0:
                ...
                if age <= 10:
                    ...
                    validity = "valid" if payload_valid else "unknown"
                    val_color = (80, 200, 160) if payload_valid else (220, 180, 80)
                    SCREEN.blit(tiny_font.render("Last command", True, (200, 200, 200)), (udp_summary_rect.x + 18, udp_summary_rect.y + 96))
                    SCREEN.blit(tiny_font.render(label_text, True, (200, 200, 200)), (udp_summary_rect.x + 18, udp_summary_rect.y + 114))
                    SCREEN.blit(tiny_font.render(f"{validity} - {age:.1f}s", True, val_color), (udp_summary_rect.x + 18, udp_summary_rect.y + 132))
                else:
                    SCREEN.blit(tiny_font.render("Commands stale (>10s)", True, (220, 160, 160)), (udp_summary_rect.x + 18, udp_summary_rect.y + 96))
                    SCREEN.blit(tiny_font.render("Open setup for details", True, (200, 200, 200)), (udp_summary_rect.x + 18, udp_summary_rect.y + 114))
            elif listener_bound:
                SCREEN.blit(tiny_font.render("Waiting for commands...", True, (160, 160, 160)), (udp_summary_rect.x + 18, udp_summary_rect.y + 96))
            else:
                SCREEN.blit(tiny_font.render("No packets yet", True, (160, 160, 160)), (udp_summary_rect.x + 18, udp_summary_rect.y + 96))

            pygame.draw.rect(SCREEN, (70, 120, 200), udp_open_rect, border_radius=14)
            SCREEN.blit(small_font.render("Open UDP setup", True, (255, 255, 255)), (udp_open_rect.x + 28, udp_open_rect.y + 14))
    ...
    if 0 < death_count < number_of_games:
        ...
        while waiting_for_input:
            ...
            ctext = "Click to continue" if "survey_language" in globals() and survey_language == "eng" else "Klicken Sie hier, um fortzufahren"
            SCREEN.blit(font.render(ctext, True, (255, 255, 255)), (continue_rect.x + 20, continue_rect.y + 25))
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    pygame.quit(); quit()
        ...
        for i in range(5, 0, -1):
            SCREEN.fill((255, 255, 255))
            SCREEN.blit(font.render(f"Next game starts in {i} seconds...", True, (0, 0, 0)), (10, SCREEN_HEIGHT // 2))
            pygame.display.flip()
    ...
    if death_count >= number_of_games:
        SCREEN.fill((255, 255, 255))
        SCREEN.blit(font.render(f"End of game - Completed {number_of_games} games", True, (0, 0, 0)), (10, SCREEN_HEIGHT // 2))
        pygame.display.flip()
