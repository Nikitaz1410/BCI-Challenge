"""
Online BCI Pipeline for the Baseline (AllRounder) Model.

This script runs real-time EEG classification using a pre-trained Baseline model.
No online adaptation is performed—the model uses the configuration trained offline.

Pipeline:
1. Load configuration and pre-trained model (baseline_best_model.pkl)
...
Requires:
- Pre-trained model from main_offline_Baseline.py (baseline_best_model.pkl)
- ArtefactRemoval from same run (artefact_removal.pkl)
- LSL streams: EEG data, optional Markers for evaluation

Usage:
    python main_online_Baseline.py
"""
...
class BCIEngine:
    ...
    def _init_models(self):
        """Load Baseline model, signal filter, and artifact rejection."""
        base_path = Path.cwd() / "resources" / "models"
        model_path = base_path / "baseline_best_model.pkl"
        artifact_path = base_path / "artefact_removal.pkl"

        if not model_path.exists():
            raise FileNotFoundError(
                f"Baseline model not found at {model_path}. "
                "Run main_offline_Baseline.py first to train and save the model."
            )
        if not artifact_path.exists():
            raise FileNotFoundError(
                f"ArtefactRemoval not found at {artifact_path}. "
                "Run main_offline_Baseline.py first."
            )

        # Signal Filter
        self.signal_filter = Filter(self.config, online=True)

        # Baseline Classifier (no online adaptation)
        self.clf = AllRounderBCIModel.load(str(model_path))
    ...
    def run(self):
        ...
        logger.info("Starting BCI Loop (Baseline, no online adaptation)...")
        ...
        try:
            while self.running:
                ...
                # --- 4. Classification (Baseline: no adaptation) ---
                # AllRounderBCIModel expects (N, C, T); add batch dimension
                window_3d = self.buffer[np.newaxis, ...]
                probs = self.clf.predict_proba(window_3d)
                probs = np.asarray(probs)  # shape (1, 3) for send_command
